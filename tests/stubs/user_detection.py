...
class UserDetect(Launcher):
    ...
    def start(self, vis_mem, frame_event, log_queue):
        ...
        if self.thread1 == None:
            ...
            self.thread1 = threading.Thread(target=worker, args=(self.stop_sig1, vis_mem, frame_event, log_queue), daemon=True)
            self.thread1.start()
    ...
    def handle_message(self, data):
        ...
        if "last_detected" in data: 
            self.last_detected.value = float(data["last_detected"])
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # Change variables
        if x == 1 :
            payload = {'last_detected' : time.time()}
